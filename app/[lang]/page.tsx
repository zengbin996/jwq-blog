    ...
    <section className="container py-16">
      ...
      {footprints.length > 0 && (
        <ul className="mt-6 flex flex-wrap gap-2">
          {footprints.map((fp) => (
            <li key={fp.id} className="px-3 py-1 rounded-full text-xs bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300">
              {fp.city ?? fp.name}
            </li>
          ))}
        </ul>
  ...
  return (
    <section className="container pt-16 pb-8">
      <div className="flex items-baseline justify-between mb-8">
        ...
        <Link href={`/${lang}/photos`} className="text-sm text-zinc-500 hover:text-black dark:hover:text-white transition-colors">
  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="flex flex-col items-center justify-center min-h-[55vh] text-center px-6 py-20">
        <h1 className="text-5xl sm:text-7xl font-bold tracking-tight text-black dark:text-white mb-5">
        ...
        <div className="flex gap-4 mt-10">
          <Link href={`/${lang}/articles`} className="px-7 py-3 rounded-full bg-black dark:bg-white text-white dark:text-black text-sm font-medium hover:opacity-80 transition-opacity">
            {dict.home.read_articles}
          </Link>
          <Link href={`/${lang}/photos`} className="px-7 py-3 rounded-full border border-black/20 dark:border-white/20 text-sm font-medium hover:bg-black/4 dark:hover:bg-white/6 transition-colors">
      ...
      <section className="bg-zinc-50 dark:bg-zinc-950 py-20">
        <div className="container">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8">
            {mottos.map((m, i) => (
              <div key={m.title} className="flex flex-col gap-4 p-6 rounded-2xl bg-white dark:bg-zinc-900 border border-black/6 dark:border-white/6">
                {MOTTO_ICONS[i]}
